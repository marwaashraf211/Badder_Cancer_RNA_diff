...
normal = as.matrix(read.csv("bladder_counts.csv", row.names=1))
ubnormal = as.matrix(read.csv("bladder_counts_f.csv", row.names=1))
...
#Ndata <- data$N.content
hist(data, col = "red", main="Data Histogram")
...
normal.mean = apply((log2(data+1))[,1:428], 1, mean)
ubnormal.mean = apply((log2(data+1))[,429:dim(data)[2]], 1, mean)
...
fold=ubnormal.mean-normal.mean
...
phenotype = as.data.frame(factor(rep(c("ubnormal","normal"), c(428,428 ))))
...
w=row_wilcoxon_twosample(data[,1:428],data[,429:dim(data)[2]])

#export the Degs into your current folder for further analysthis
write.csv(as.matrix(res.deg),file="nonparameric.csv", quote=F,row.names=T)
